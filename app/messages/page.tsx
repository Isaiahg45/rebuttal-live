...
function MessagesInner() {
  ...
  useEffect(() => {
    ...
    supabase.from('buddies')
      ...
      .then(({ data }) => {
        if (data) {
          const list = data.map(b => b.user_a === profile.username ? b.user_b : b.user_a)
          setBuddies(list)
        }
      })
  }, [profile?.username])
  ...
  const loadConversations = async () => {
    ...
    if (usernames.length > 0) {
      ...
      if (profileData) {
        ...
        profileData.forEach(p => { if (p.avatar_url) av[p.username] = p.avatar_url })
  ...
  useEffect(() => {
    loadConversations()
    pollRef.current = setInterval(loadConversations, 4000)
  ...
  const sendMessage = async () => {
    if (!input.trim() || !profile?.username || !activeConvo) return
  ...
  const startNewConvo = async () => {
    const target = newRecipient.trim().toLowerCase()
      ...
      <div style={{ height: 'calc(100vh - 56px)', display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{ width: '320px', flexShrink: 0, borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', background: 'var(--surface)' }}>
          ...
          <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <button onClick={() => setShowNewMsg(true)} style={{ background: 'var(--accent)', border: 'none', borderRadius: '8px', padding: '6px 12px', color: '#fff', fontSize: '12px', fontWeight: 700, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>
              + New
            </button>
          </div>
          ...
          {/* New message input */}
          {showNewMsg && (
            <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)', background: 'rgba(230,57,70,0.06)' }}>
              <div style={{ fontSize: '11px', color: 'var(--muted)', marginBottom: '6px', fontWeight: 600 }}>Send to username:</div>
              <div style={{ display: 'flex', gap: '6px' }}>
                ...
                />
                <button onClick={startNewConvo} style={{ background: 'var(--accent)', border: 'none', borderRadius: '6px', padding: '6px 10px', color: '#fff', fontSize: '12px', fontWeight: 700, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>Go</button>
                <button onClick={() => setShowNewMsg(false)} style={{ background: 'none', border: 'none', color: 'var(--muted)', cursor: 'pointer', fontSize: '16px' }}>✕</button>
              </div>
            </div>
          )}
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            ...
            {conversations.map(convo => (
              ...
              >
                {/* Avatar */}
                <div style={{ width: '42px', height: '42px', borderRadius: '50%', flexShrink: 0, overflow: 'hidden', background: 'linear-gradient(135deg,var(--accent),#ff8c69)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px', fontWeight: 700, color: '#fff', position: 'relative' }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              {messages.map((msg, i) => {
                ...
                const showName = i === 0 || messages[i - 1].sender_username !== msg.sender_username
                return (
                  <div key={msg.id} className="msg-bubble" style={{ display: 'flex', flexDirection: isMe ? 'row-reverse' : 'row', alignItems: 'flex-end', gap: '8px' }}>
                    {!isMe && showName && (
                      <div style={{ width: '28px', height: '28px', borderRadius: '50%', overflow: 'hidden', background: 'linear-gradient(135deg,#a855f7,#7c3aed)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '9px', fontWeight: 700, color: '#fff', flexShrink: 0, marginBottom: '2px' }}>
                    ...
                    {!isMe && !showName && <div style={{ width: '28px', flexShrink: 0 }} />}
            ...
            <div style={{ padding: '12px 20px', borderTop: '1px solid var(--border)', background: 'var(--surface)', flexShrink: 0, display: 'flex', gap: '10px', alignItems: 'flex-end' }}>
              ...
              <button
                onClick={sendMessage}
                disabled={!input.trim()}
                style={{ background: 'var(--accent)', border: 'none', borderRadius: '50%', width: '40px', height: '40px', color: '#fff', fontSize: '16px', cursor: input.trim() ? 'pointer' : 'not-allowed', opacity: input.trim() ? 1 : 0.4, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
              >
                ➤
              </button>
            </div>
